...
const LoginPage = () => {
  ...
  const [sucessError, setSucessError] = useState("");
  ...
  const handleForm = (e) => {
    ...
    if (!PasswordValidate(password)) {
      ...
      setEmailErr("Your Email not valid");
    }
    signInWithEmailAndPassword(auth, email, password)
      .then((result) => {
        console.log(result.user);
        if (result.user.emailVerified) {
          setSucessLogin("Successfully done");
        } else {
          alert("please email varifide");
        }
      })
      .catch(() => setSucessError("Please your email and password"));
    setInputData({
      email: "",
      password: "",
    });
  ...
  const handleForgetPassword = () => {
    console.log("This is handle forget pass", email);
      ...
      <div className="w-full max-w-md p-8 space-y-3 rounded-xl bg-[#2B3440] text-gray-100">
        ...
        <form onSubmit={handleForm} className="space-y-6">
          ...
          <div className="space-y-1 text-sm">
            ...
            />
            {passwordErr && (
              <p className="text-red-500 text-xs mt-1">{passwordErr}</p>
            )}
            <div className="flex justify-end text-xs cursor-pointer text-gray-400 underline">
        ...
        {sucessError && (
          <p className="text-red-500 text-xs mt-1 text-center">{sucessError}</p>
        )}
        <div className="flex items-start pt-4 space-x-1">
          <div className="flex-1 h-px sm:w-16 bg-gray-700"></div>
          <p className="px-3 text-sm text-gray-400">
            {loginUsers ? loginUsers?.email : "Do not have an account?"}
          </p>
          <p>{loginUsers?.displayName}</p>
          <img className="w-[50px]" src={loginUsers?.photoURL}></img>
